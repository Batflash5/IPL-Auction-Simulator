...
//matpl = matches _played; runs = runs scored
//hacen = HalfCenturies; cen = centuries
//high = highest score; bid = bidamount
//finame = first name; laname = last name
//debut = debut year; type= left or right handed
//past = past team(before bidding); cur = current team(after bidding)

struct playerbidbat                                             //Defines a structure for attributes of batsmen
{
...
//matpl = matches _played; wickets= wickets took
//haul3 = 3 wicket haul; haul5 = 5 wicket haul
//bid = bidamount;  base = base price
//finame = first name; laname = last name
//debut = debut year; cur = current team(after bidding) 
//past = past team(before bidding)
...
//RTM - remaining Right To Match
//tcode - Like KKR,CSK.etc
//budget = Budget allocated for each team 
...
void buddisp(struct team i[7])   //Budget Display
{
...
void rtmdisp(struct team i[7])   //RTM Display
{
...
//Copying the details to the structure object(BATSMEN)
...
//Copying the details to the structure object(BOWLERS)
...
//Copying the details to the structure object(ALL Rounders)
...
//Displaying player details(Batsmen)
...
//Displaying player details(Bowlers)
...
//Displaying player details(All Rounders)
...
//Decreasing Budgets and RTM(if used)(Batsmen)
...
//Decreasing Budgets and RTM(if used)(Bowlers)
...
//Decreasing Budgets and RTM(if used)(All rounders)
...
//calling display functions and all other important things
